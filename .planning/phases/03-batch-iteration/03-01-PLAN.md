...
must_haves:
  ...
  artifacts:
    ...
    - path: "utils/queue_control.py"
      provides: "Queue triggering utilities"
      exports: ["trigger_next_queue", "HAS_SERVER"]
    - path: "tests/test_iteration_state.py"
      provides: "Unit tests for iteration state"
      min_lines: 50
    - path: "tests/test_queue_control.py"
      provides: "Unit tests for queue control"
      min_lines: 30
  key_links:
...
Purpose: These utilities provide the foundation for tracking position across workflow executions and triggering the next queue item when batch continues.

Output: Two utility modules with comprehensive tests - `iteration_state.py` for state tracking and `queue_control.py` for queue triggering.
</objective>
...
<task type="auto">
  <name>Task 2: Create queue control utility</name>
  <files>utils/queue_control.py, tests/test_queue_control.py</files>
  <action>
Create `utils/queue_control.py` with queue triggering utilities.
...
Use unittest.mock to mock PromptServer for testing the trigger path.
  </action>
  <verify>Run `python -m pytest tests/test_queue_control.py -v` - all tests pass</verify>
  <done>Queue control utilities work correctly, with graceful degradation when PromptServer unavailable</done>
</task>
...
- Import and export `trigger_next_queue`, `should_continue`, `HAS_SERVER` from `queue_control`

Follow the existing pattern in utils/__init__.py for how exports are structured.
  </action>
  <verify>Run `python -c "from comfyui_batch_image_processing.utils import IterationState, trigger_next_queue, should_continue, HAS_SERVER; print('OK')"` from project root</verify>
  <done>New utilities are importable from the utils package</done>
</task>
